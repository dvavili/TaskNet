...
//                                                        - (Preferences.host_reserved_load + host_node.getPromisedLoad());
//                                            }
...
//                                }
                                ...
                                switch (msg.getNormalMsgType()) {
                                    ...
                                    case DISTRIBUTED_TASK:
                                        ...
                                        if (host_node.getAcceptedTaskByTaskId(taskChunk.getTaskAdvReplyId()) != null) {
                                            host_node.decrPromisedLoad(host_node.getAcceptedTaskByTaskId(
                                        ...
                                        try {
                                            ...
                                            Logger.getLogger(
                                                    TaskDistributor.class.getName()).log(Level.SEVERE, null, ex);
                                        }
                                        break;
                                    case TASK_RESULT:
                                        ...
                                        synchronized (taskLookup) {
                                            taskLookup.getTaskGroup().get(taskLookup.getResultTracker().get(taskResult.getSeqNumber())).setStatus(
            ...
            private Serializable handleDistributedTask(DistributedTask gotTask) {
                try {
                    ...
                    if (gotTask.getParameters() != null
                            && ((mthdDef.get(gotTask.getMethodName())).length != gotTask.getParameters().length)) {
            ...
            private void distributeTask(TaskAdvReply taskAdvReply) {
                ...
                int loadDistributed = (int) Math.ceil((taskToDistribute.getTaskLoad() > taskAdvReply.getLoadCanServe()) ? (taskAdvReply.getLoadCanServe()) : taskToDistribute.getTaskLoad());
        ...
        (new Thread() {
            ...
            public void run() {
                while (taskChunk.getStatus() != Preferences.TASK_CHUNK_STATUS.RECEIVED
                    ...
                    try {
                        taskLookups.get(
                                ((DistributedTask) taskToDistribute).getTaskId()).addToResultTracker(
                                taskChunk.getSequenceNumber(),
