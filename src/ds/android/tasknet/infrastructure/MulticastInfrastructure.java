...
//                                            }
                                ...
                                switch (msg.getNormalMsgType()) {
                                    ...
                                    case TASK_RESULT:                                    	
                                        ...
                                        synchronized(taskLookups) {
                                        	taskLookup.getTaskGroup().get(seqNumber).setStatus(Preferences.TASK_CHUNK_STATUS.RECEIVED);
                                        }
            ...
            private void distributeTask(TaskAdvReply taskAdvReply) {
                    ...
                    taskLookup.addToTaskGroup(taskChunk.getSequenceNumber(), taskChunk);
                    Message distMsg = new Message(node.getName(), "", "", taskChunk);
                    distMsg.setNormalMsgType(Message.NormalMsgType.DISTRIBUTED_TASK);
         
                    sendAndRetryTaskChunk(taskChunk, distMsg, taskToDistribute, loadDistributed, taskLookup);
    ...
    private void sendAndRetryTaskChunk(final TaskChunk taskChunk, final Message distMsg, 
    		final Task taskToDistribute, final int loadDistributed, final TaskLookup taskLookup) {
        ...
        (new Thread() {
        	public void run() {
        		//synchronized (taskLookup) {
                    ...
                    while(taskLookup.getStatus() != Preferences.TASK_STATUS.DISTRIBUTED
